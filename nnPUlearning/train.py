...
from model import LinearClassifier, ThreeLayerPerceptron, MultiLayerPerceptron, CNN, ModelCardiotocography, ModelFAHZU, ModelMIMIC
...
def process_args(arguments):
    ...
    parser.add_argument('--fold', '-f', type=int, default=None,
                        help='the fold number for k-fold validation')
    parser.add_argument('--split', '-sp', type=int, default=None,
                        help='the split number for LORS')
    parser.add_argument('--dim', type=int, default=None,
                        help='the dim for prediction')
    ...
    parser.add_argument('--dataset', '-d', default='mnist', type=str, choices=['mnist', 'cifar10', 'cardiotocography', 'FAHZU', 'MIMIC'],
    ...
    parser.add_argument('--model', '-m', default='3lp', choices=['linear', '3lp', 'mlp', 'cardiotocography', 'FAHZU', 'MIMIC'],
...
def select_model(model_name):
    models = {"linear": LinearClassifier, "3lp": ThreeLayerPerceptron,
              ...
              "cardiotocography": ModelCardiotocography,
              "FAHZU": ModelFAHZU, "MIMIC": ModelMIMIC}
...
def main(arguments):
    ...
    # dataset setup
    XYtrain, XYtest, prior = load_dataset(args.dataset, args.labeled, args.unlabeled, fold=args.fold, split=args.split, dim=args.dim)
    dim = XYtrain[0][0].size // len(XYtrain[0][0])
    if np.array(XYtrain[0][0]).ndim == 1:
        dim = len(XYtrain[0][0])
    ...
    import pandas as pd
    nnPU_output = pd.DataFrame(models['nnPU'].calculate(np.stack([item[0] for item in XYtest])).array)
    nnPU_output.to_csv(("./output/%s/nnPU_valid" % args.dataset) + 
                       ("_fold_%d" % args.fold if args.fold is not None else "") +
                       ("_split_%d" % args.split if args.split is not None else "") + 
                       ("_dim_%d" % args.dim if args.dim is not None else "") + 
                       ".csv", index=False)

    uPU_output = pd.DataFrame(models['uPU'].calculate(np.stack([item[0] for item in XYtest])).array)
    uPU_output.to_csv(("./output/%s/uPU_valid" % args.dataset) + 
                      ("_fold_%d" % args.fold if args.fold is not None else "") +
                      ("_split_%d" % args.split if args.split is not None else "") + 
                      ("_dim_%d" % args.dim if args.dim is not None else "") +
                      ".csv", index=False)

    target_output = pd.DataFrame(np.stack([item[1] for item in XYtest]))
    target_output.to_csv(("./output/%s/target" % args.dataset) + 
                         ("_fold_%d" % args.fold if args.fold is not None else "") +
                         ("_split_%d" % args.split if args.split is not None else "") + 
                         ("_dim_%d" % args.dim if args.dim is not None else "") + 
                         ".csv", index=False)
