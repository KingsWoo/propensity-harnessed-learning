...
def make_dataset(dataset, n_labeled, n_unlabeled):
    def make_pu_dataset_from_binary_dataset(x, y, labeled=n_labeled, unlabeled=n_unlabeled):
        ...
        perm = np.random.permutation(len(y))
        x, y = x[perm], y[perm]
        print('positive sample number:', (y == positive).sum())
        return x, y, _prior
...
def get_cardiotocography(fold, split, path='./data/cardiotocography'):

    train_inputs = pd.read_csv(os.path.join(path, "train_inputs_fold_%d_split_%d.csv" % (fold, split))).values
    train_labels = pd.read_csv(os.path.join(path, "train_labels_fold_%d_split_%d.csv" % (fold, split))).values
    valid_inputs = pd.read_csv(os.path.join(path, "valid_inputs_fold_%d_split_%d.csv" % (fold, split))).values
    valid_labels = pd.read_csv(os.path.join(path, "valid_labels_fold_%d_split_%d.csv" % (fold, split))).values
    
    return (train_inputs, train_labels[:,0]), (valid_inputs, valid_labels[:,0])

def get_fahzu(fold, split, dim, path='./data/FAHZU'):
    ...
    return (train_inputs, train_labels[:,dim]), (valid_inputs, valid_labels[:,dim])

def get_mimic(fold, split, dim, path='./data/MIMIC'):
...
def load_dataset(dataset_name, n_labeled, n_unlabeled, fold=None, split=None, dim=None):
    if dataset_name == "mnist":
        ...
        (x_train, y_train), (x_test, y_test) = get_cardiotocography(fold, split)
    elif dataset_name == "MIMIC":
        (x_train, y_train), (x_test, y_test) = get_mimic(fold, split, dim)
    elif dataset_name == "FAHZU":
        (x_train, y_train), (x_test, y_test) = get_fahzu(fold, split, dim)
    else:
